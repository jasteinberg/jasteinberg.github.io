...
almost every issue studied in this post can be traced back to the gap between an estimate and its population target, $$\hat\theta$$ and $$\theta$$, or the eigenvalues of $$\hat C$$ and of $$\Sigma$$. I fit quantities on a training split of
...
These population quantities have no closed form: they are the
$$N_{\text{train}}\to\infty$$ limits under the data-generating distribution and are never
obtained directly. Their influence is visible only through held-out performance against the random-direction null. The one exception is the whitening schematic below, whose activations are drawn from a *known* Gaussian, so there
$$\Sigma$$ and the optimal direction $$\Sigma^{-1}\delta$$ are known by construction rather
than estimated.

## Probes, estimators and scores
...
**Mass-mean probes from the difference-in-means direction.** The mass-mean direction is defined as
...
$$\hat\Sigma = (1-\rho)\,\hat C + \rho\,\frac{\operatorname{tr}\hat C}{d} I$$,
...
$$\mathrm{AUROC}$$ and $$d'$$ are linked when the class-conditionals are Gaussian and the classes balanced, so that $$\Sigma$$ is the plain average of the two class covariances:

$$
\mathrm{AUROC} \;=\; \Phi\!\left(\frac{d'}{\sqrt{2}}\right),
$$
...
I build both nulls in this post from *random unit vectors*. As mentioned in the previous section, to sample $$u$$ uniformly from the unit sphere
...
**The decoding null.** A random direction does not achieve $$\mathrm{AUROC} = \tfrac{1}{2}$$ merely up to
...
null, evaluate them on the *same held-out points*, and fold as above.

**Whitening.** $$\hat\Sigma$$ as defined above. At $$N_{\text{train}}
\approx 600$$ against $$d = 2560$$ the shrinkage is what makes the inverse usable.

## Regime of recoverability

With the null, $$d'$$, and the class-mean-gap unit defined, at what scale, at what depth, and across which datasets is a truth direction actually recoverable?

For both `cities` and `counterfact` the held-out AUROC at the best layer shows that recovery occurs starting at pythia-410m. For `cities`, the best-layer recovery can be done via mass-mean probe or whitened probe while for `counterfact` recovery requires a whitened probe for pythia-410m.

![Emergence of the truth direction across the Pythia scale ladder for `cities` (left) and `counterfact` (right). Each panel shows the held-out AUROC of the mass-mean direction (red) and the whitened direction (blue) against the random-direction null (gray band, up to its 95th percentile). At each model size both are read at the layer which maximizes the mass-mean direction's margin above its null. At 70m both the mass-mean and whitened directions sit inside the null for both datasets. For `cities` both directions clear the null from 410m on and the mass-mean margin widens with $$d_{\text{model}}$$. For `counterfact` the whitened direction is the only one that clears the null for every model in the range 410m to 2.8b, and the mass-mean margin peaks at 1.4b and falls at 2.8b.](/assets/figures/truth_emergence.png){: .fig-single}
...
![Cross-dataset transfer of the mass-mean direction across the nine main-tier sets, pythia-2.8b. Each direction is fit on its source's training half at the source's selected layer and scored on each target's held-out half at the target's selected layer, so the diagonal is held-out too. Rows are the fitting set, columns the evaluation set. The color map diverges about chance because the below-chance cells are a finding, not noise.](/assets/figures/truth_transfer.png){: .fig-single}
...
synthetic control in *Empirical setup* puts its overlap with the true direction at $$0.24$$ for
...
The slope of behavioral response against steering coefficient is the *steerability* of [Tan et al.
(2024)](https://arxiv.org/abs/2407.12404), who fit this line through a
logit-difference propensity curve. While their intervention displaces the last token
position only, the harness here displaces every position, so magnitudes are not
directly comparable. In this post, I additionally split the steering response into even and odd
...
that of a random direction, and the spread above that median, $$p_{95}$$ from $$0.094$$ to $$0.139$$, is what the two percent of variance off the axis contributes. That is what aligned with the axis means
...
On `counterfact` the leading axis of the
...
rogue dimension is absent at 1.4b and present at 2.8b, so the larger model is the one
...
Tegmark's assignment in *Separability, capacity and readout* less paradoxical than it first appears. A
...
Four recent reports also examine steering failures in fitted directions. Here I compare the
rogue-dimension account against each.
...
directions: domain-specific directions, and a single
*domain-general* direction recovered by concept erasure across many truth types. They find steering
along a domain-specific direction improves truthfulness on held-out factual questions but
steering along the domain-general direction consistently degrades performance. Their account is one
...
That mechanism is not available here, since the estimator showing the steering failure is fitted on the
`counterfact` domain alone and with no second domain to mix in. The result in their study and the result in this post therefore agree that a direction fitted to truth can exhibit incorrect behavior during steering but disagree on the mechanism and on the remedy. 
...
from the low-discriminability failures Braun et al. describe. The steering inversion they observe is along a direction that decodes well while the `counterfact` steering inversion I observe is shown to steer along a nuisance direction that fails to decode, since the estimator $$\hat\theta$$ sits inside its decoding null. The mechanism found in this post, alignment with a label-blind variance axis, is unavailable when
...
unchanged. Unlike accuracy, it needs no choice of threshold.
...
case the relationship between $$d'$$ and AUROC is only approximate, which is why the main text
...
mid-depth. What separates the other two from `counterfact` is that their class gap
grows enough to leave the axis by the later layers of the network.

One caution about the `companies_true_false` row is that $$d'$$ of $$0.17$$ maps to $$\mathrm{AUROC} \approx 0.55$$
under the Gaussian identity, not the $$0.699$$ measured. This is the largest
...
$$d'_{\mathrm{mm}}$$ stays inside its null under every regime. The layer-$$28$$ row is the informative one. Removing the eleven moves
...
$$\rho$$ on the uncleaned data reproduces most of the effect of deleting the eleven statements:
...
target that the inverse barely downweights the leading directions.
...
the one every result is scored against. The steering null
...
**Layer selection.** The reported layer maximizes the margin $$m$$ of *Separability, capacity and readout* —
...
$$\hat\lambda_1/\hat\lambda_2$$. This is consistent with the class gap carrying most of
...
Ledoit–Wolf's shrinkage intensity to estimate $$\Sigma$$, which minimizes
